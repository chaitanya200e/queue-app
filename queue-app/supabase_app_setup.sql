...
insert into public.admins (email, name, domain, approved_by, approved_at)
values ('chaitanyamandale125@gmail.com', 'Super Admin', 'general', 'system', now())
...
create policy "Queue public insert"
...
create policy "Slots public update booking"
on public.slots for update
using (true)
with check (true);

create policy "Slots admins insert"
...
create policy "Slots admins update"
on public.slots for update
to authenticated
using (
  lower(auth.jwt() ->> 'email') = 'chaitanyamandale125@gmail.com'
...
create policy "Counters public read"
on public.counters for select
using (true);

create policy "Counters public update"
on public.counters for update
using (id = 'tokenCounter')
with check (id = 'tokenCounter');

create policy "Meta public read"
on public.meta for select
using (true);

create policy "Meta admins update"
...
create policy "Admins self or super read"
...
create policy "Admin requests public insert"
on public.admin_requests for insert
with check (true);

create policy "Super admin read requests"
on public.admin_requests for select
to authenticated
using (lower(auth.jwt() ->> 'email') = 'chaitanyamandale125@gmail.com');

create policy "Super admin update requests"
